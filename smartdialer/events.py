...
from smartdialer.agent_assignment import SIM_TIME_SCALE
from smartdialer.enums import CallStatus, EventClassification
from smartdialer.transitions import (
    classify_call_event, EVENT_TARGET_STATUS, TERMINAL_EVENT_TARGET, borrower_status_for_call,
)
...
def ingest_event(conn, event, call_id: str | None) -> EventClassification:
    ...
    if classification == EventClassification.VALID:
        _apply_valid_event(conn, call_id, event.event_type, agent_id, current_status)
...
def _apply_valid_event(conn, call_id: str, event_type: str, agent_id: int | None,
    ...
    if event_type in TERMINAL_EVENT_TARGET and current_status == CallStatus.AWAITING_AGENT:
        # fix #4: a call that was waiting for an agent and then the provider says it's over
        # means the borrower hung up / the call ended without ever reaching an agent — that
        # is exactly what ABANDONED means, not a literal COMPLETED/FAILED/CANCELLED.
        target = CallStatus.ABANDONED
    conn.execute(text(
    ...
    if event_type in TERMINAL_EVENT_TARGET:
        ...
        # fix #5: close the borrower lifecycle on every terminal call outcome — CALLED on
        # success, back to PENDING on failure/abandonment so a later dial attempt can retry.
        borrower_status = borrower_status_for_call(target)
